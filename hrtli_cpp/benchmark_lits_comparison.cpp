...
#else
// LITS vendors a modified HOT with the same namespaces and include guards.
// Compile it separately: including both would silently substitute that fork.
...
int main(int argc, char** argv) {
    try {
        require(argc == 11 || argc == 12, "usage: comparison MODE BASE_TXT BASE_NUL ARRIVALS_NUL BASE_N ARRIVAL_N OPS WRITE_PERCENT SEED EPSILON [attempts|valid_writes|prefix_bursts]");
        ...
        require(family == "attempts" || family == "valid_writes" || family == "prefix_bursts", "unknown workload family");
        ...
        if (family != "attempts") {
            ...
            active.reserve(base_count);
            inactive.reserve(arrival_count + 1);
            // A fixed, natural reversed-host prefix is chosen before timing.
            // Only writes are clustered; reads retain the full population.
            for (size_t i = 0; i < base_count; ++i)
                if (family != "prefix_bursts" || base.view(i).substr(0, 4) == "com.") active.push_back(i);
            for (size_t i = 0; i < arrival_count; ++i)
                if (family != "prefix_bursts" || arrivals.view(i).substr(0, 4) == "com.") inactive.push_back(base_count + i);
            require(!active.empty() && !inactive.empty(), "no eligible clustered write keys");
            if (family == "prefix_bursts") {
                require(write_percent == 50 && operation_count % 128 == 0, "prefix bursts require full 128-operation blocks and 50 percent writes");
                // Consecutive 64-write bursts alternate with 64 full-pool reads.
                for (size_t i = 0; i < operations.size(); ++i) operations[i].type = i % 128 < 64 ? 1 : 0;
            }
            bool deleting = true;
        ...
        for (auto& op : operations) {
            ...
            require(family == "attempts" || op.type == 0 || op.expected,
                    "eligible-set generator produced an invalid write");
            require(family != "prefix_bursts" || op.type == 0 || op.key.compare(0, 4, "com.") == 0,
                    "write escaped the declared natural prefix");
...
#else
        else run<LitsAdapter>(argv[1], base, arrivals, argv[2], epsilon, operations, live, seed, write_percent, hash, family);
#endif
        return 0;
