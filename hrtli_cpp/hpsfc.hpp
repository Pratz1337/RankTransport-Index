...
inline uint64_t sparse_lexicode_hash(const std::string& key) {
    ...
    for (unsigned char c : key) {
        uint16_t code = static_cast<uint16_t>(c) + 1U;
        hash = fnv1a_mix_byte(hash, static_cast<uint8_t>(code & 0xffU));
        hash = fnv1a_mix_byte(hash, static_cast<uint8_t>((code >> 8) & 0xffU));
    }
