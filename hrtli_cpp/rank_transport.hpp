...
#define DeltaLayerType CountedBTreeDelta
...
public:
    ...
    int inserted_size() const {
        int total = 0;
        for (auto* ptr : local_deltas) if (ptr) total += ptr->inserted_size();
        for (const auto& entry : write_buffer) {
            int seg_idx = entry.seg_idx;
            int committed_w = 0;
            if (local_deltas[seg_idx]) {
                committed_w = local_deltas[seg_idx]->contains_inserted(entry.key) ? 1 : 
                              (local_deltas[seg_idx]->contains_deleted(entry.key) ? -1 : 0);
            }
            if (entry.weight == 1) {
                if (committed_w == 0) {
                    total += 1;
    ...
    int deleted_size() const {
        int total = 0;
        for (auto* ptr : local_deltas) if (ptr) total += ptr->deleted_size();
        for (const auto& entry : write_buffer) {
            int seg_idx = entry.seg_idx;
            int committed_w = 0;
            if (local_deltas[seg_idx]) {
    ...
    int size() const {
        return inserted_size() + deleted_size();
    }
    ...
    bool contains_inserted(const std::string& key, int seg_idx) const {
        ...
        if (seg_idx < 0 || seg_idx >= (int)P || !local_deltas[seg_idx]) return false;
        return local_deltas[seg_idx]->contains_inserted(key);
    }
    ...
    bool contains_deleted(const std::string& key, int seg_idx) const {
        ...
        if (seg_idx < 0 || seg_idx >= (int)P || !local_deltas[seg_idx]) return false;
        return local_deltas[seg_idx]->contains_deleted(key);
    }
...
private:
    ...
    mutable SegmentedDelta<CountedBTreeDelta> delta;
...
public:
    RankTransportIndex(const std::vector<std::string>& initial_keys, int eps)
        : base_epsilon(eps) {
    ...
    bool contains(const std::string& key) const {
        ...
        if (delta.contains_inserted(key, seg_idx)) return true;
        return contains_base_under_lock(key) && !delta.contains_deleted(key, seg_idx);
    }
    ...
    int lookup(const std::string& key) const {
        ...
        // When no mutations have accumulated, skip the delta layer entirely.
        // This is the common case in read-heavy workloads and eliminates the
        // prefix_delta call cost for a clean base state.
        if (delta.empty()) {
        ...
        if (delta.contains_inserted(key, seg_idx)) {
            auto it = std::lower_bound(base_keys.begin(), base_keys.end(), key);
            int base_less = static_cast<int>(
                std::distance(base_keys.begin(), it));
            int res = base_less + delta.prefix_delta(key, seg_idx);
#ifdef HRTLI_VERIFY_ORACLE
    ...
    std::vector<std::string> scan_range(const std::string& lo, const std::string& hi) const {
        ...
        std::vector<SegmentedDelta<CountedBTreeDelta>::BufferEntry> buf_entries;
        auto buf_start = std::lower_bound(delta.get_write_buffer().begin(), delta.get_write_buffer().end(), SegmentedDelta<CountedBTreeDelta>::BufferEntry{lo, 0, 0});
        auto buf_end = std::upper_bound(delta.get_write_buffer().begin(), delta.get_write_buffer().end(), SegmentedDelta<CountedBTreeDelta>::BufferEntry{hi, 0, 0});
    ...
    size_t consolidate() {
        ...
        rebuild_thread = std::thread([this, snapshot]() {
...

            {
                ...
                SegmentedDelta<CountedBTreeDelta> new_segmented_delta(this->model.segments.size());
