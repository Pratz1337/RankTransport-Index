...
private:
    struct Node {
        ...
        // Small nodes scan at most 16 children; wider nodes own a Fenwick array.
        std::unique_ptr<std::vector<int>> child_mass;
    };
    ...
    static int earlier_mass(const Node* node, size_t count) {
        int sum = 0;
        if (!node->child_mass) {
            for (size_t i = 0; i < count; ++i) sum += node->children[i]->subtree_sum;
            return sum;
        }
        while (count) {
            sum += (*node->child_mass)[count - 1];
            count &= count - 1;
        }
        return sum;
    }

    static void add_child_mass(Node* node, size_t index, int diff) {
        if (!node->child_mass) return;
        for (; index < node->child_mass->size(); index |= index + 1)
            (*node->child_mass)[index] += diff;
    }

    static void reserve_child_mass(Node* node, size_t count) {
        if (node->child_mass) {
            node->child_mass->reserve(count);
        } else if (count > 16) {
            // Complete a valid cache of the old topology before publishing it.
            // If child insertion subsequently throws, the old tree still works.
            auto mass = std::make_unique<std::vector<int>>();
            mass->reserve(count);
            for (const auto& child : node->children) mass->push_back(child->subtree_sum);
            for (size_t i = 0; i < mass->size(); ++i) {
                size_t parent = i | (i + 1);
                if (parent < mass->size()) (*mass)[parent] += (*mass)[i];
            }
            node->child_mass = std::move(mass);
        }
    }

    // Call only after reserving capacity before any live tree mutation.
    static void rebuild_child_mass(Node* node) {
        if (node->children.size() <= 16) {
            node->child_mass.reset();
            return;
        }
        auto& mass = *node->child_mass;
        mass.resize(node->children.size());
        for (size_t i = 0; i < node->children.size(); ++i)
            mass[i] = node->children[i]->subtree_sum;
        for (size_t i = 0; i < node->children.size(); ++i) {
            const size_t parent = i | (i + 1);
            if (parent < mass.size())
                mass[parent] += mass[i];
        }
    }
    ...
    static int assign(Node* node, const std::string& key, size_t pos, int new_weight) {
        ...
        if (matched == child->edge.size()) {
            ...
            node->subtree_sum += diff;
            add_child_mass(node, index, diff);
            if (normalize_child(node, index)) rebuild_child_mass(node);
            return diff;
        }
        ...
        rebuild_child_mass(split.get());
        node->children[index] = std::move(split);
        node->subtree_sum += new_weight;
        add_child_mass(node, index, new_weight);
        return new_weight;
    }

    static bool normalize_child(Node* parent, size_t index) {
        ...
        if (child->weight == 0 && child->children.size() == 1) {
            ...
            child->child_mass = std::move(only->child_mass);
        }
        return false;
    }
    ...
    static size_t memory_bytes_node(const Node* node) {
        ...
        if (node->child_mass)
            total += sizeof(std::vector<int>) + node->child_mass->capacity() * sizeof(int);
    ...
    static std::pair<bool, int> validate_node(const Node* node) {
        int sum = node->weight;
        int previous = -1;
        if (node->child_mass && node->child_mass->size() != node->children.size()) return {false, 0};
        if (!node->child_mass && node->children.size() > 16) return {false, 0};
        size_t count = 0;
        for (const auto& child : node->children) {
            ...
            sum += result.second;
            if (earlier_mass(node, ++count) != sum - node->weight) return {false, 0};
        }
