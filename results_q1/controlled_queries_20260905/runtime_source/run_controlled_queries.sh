...
for trial in $(seq "$first_trial" "$last_trial"); do
    ...
    python3 scripts/record_experiment.py --output "$record" \
        --source scripts/record_experiment.py --source scripts/run_controlled_queries.sh \
        ...
        -- taskset -c 2 "$binary" "$base" "$insert" \
