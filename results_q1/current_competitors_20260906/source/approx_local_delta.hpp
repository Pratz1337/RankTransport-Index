...
inline double hli_key_to_double(const std::string& path_str) {
    // double is enough for the 24-byte prefix used as a model feature.
    // long double was 2-3x slower on the segment_of hot path without tightening
    // the certified epsilon (still derived from rank residuals, not this float).
    double value = 0.0;
    double denom = 1024.0;
    size_t limit = std::min<size_t>(path_str.size(), 24);
    for (size_t i = 0; i < limit; ++i) {
        unsigned int code = static_cast<unsigned char>(path_str[i]) + 1U;
        value += static_cast<double>(code) / denom;
        denom *= 1024.0;
    }
    return value;
}
