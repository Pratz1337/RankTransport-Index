...
public:
    MappedKeyStore(const std::string& path, size_t expected_count)
        ...
        try {
            ...
            // A throwing constructor has no destructor: release the mapping and
            // descriptor even when allocation of the offset vector fails.
...
public:
    ...
    // recovered position brackets the query; otherwise search its exact segment.
    size_t lower_bound(std::string_view key, const MappedKeyStore& keys,
        ...
        const size_t routed = segment_of(key, keys);
        const Segment& segment = segments_[routed];
        // A repaired constant block has a midpoint anchor, not a start index.
        const size_t first = routed == 0 ? 0 : segments_[routed - 1].end_index + 1;
        const size_t end = segment.end_index + 1;
        const int64_t predicted = predict_segment(segment, packed_key_to_double(key));
        size_t lo = static_cast<size_t>(std::clamp<int64_t>(predicted - epsilon_, first, end));
        size_t hi = static_cast<size_t>(std::clamp<int64_t>(predicted + epsilon_ + 1, first, end));
        while (lo < hi) {
            const size_t mid = lo + (hi - lo) / 2;
...
public:
    // Hash-free learned recovery is the default. The optional fingerprint table
    // affects membership only; ordered queries continue to use learned recovery.
    PackedRankTransportIndex(const std::string& base_path, size_t expected_count,
                             int epsilon, bool build_fingerprints = false)
    ...
    bool binary_point_lookup(const std::string& key) const {
        if (delta_.contains_inserted(key)) return true;
        if (delta_.contains_deleted(key)) return false;
    ...
    // Normal ordered queries use the certified window and exact boundary guard.
    // Full binary search remains an explicit control, not the default recovery.
    int exact_rank(const std::string& key) const {
        return learned_exact_rank(key);
    }

    int count_range(const std::string& lo, const std::string& hi) const {
        return learned_count_range(lo, hi);
    }

    int binary_exact_rank(const std::string& key) const {
    ...
    int binary_count_range(const std::string& lo, const std::string& hi) const {
        if (lo > hi) return 0;
    ...
    // Stop-the-world materialization: callers must not mutate this index from
    // the visitor or another thread. Views are valid only during each call.
    // This does not publish a new generation or clear the old ledger; a failed
    // writer/rebuild therefore leaves the current logical state unchanged.
    template<class Visitor>
    size_t for_each_live(Visitor visit) const {
        const auto inserted = delta_.inserted_list();
        const auto deleted = delta_.deleted_list();
        size_t i = 0, j = 0, d = 0, count = 0;
        while (i < base_.size() || j < inserted.size()) {
            if (i < base_.size() && d < deleted.size() && base_.view(i) == deleted[d]) {
                ++i;
                ++d;
                continue;
            }
            if (j < inserted.size() &&
                (i == base_.size() || std::string_view(inserted[j]) < base_.view(i))) {
                visit(std::string_view(inserted[j++]));
            } else {
                visit(base_.view(i++));
            }
            ++count;
...
private:
    bool learned_base_contains(const std::string& key) const {
        const size_t pos = model_.lower_bound(key, base_);
        return pos < base_.size() && base_.view(pos) == key;
    }
