...
static void check(const char* name, const Corpus& base, const char* path,
    ...
    verify();
    std::cout << name << " PASS: " << keys.size() << " prefix/ASCII/254-byte keys, 30000 mixed operations, full-state checks" << std::endl;
}

int main(int argc, char** argv) {
    try {
        require(argc == 3, "usage: adapter_test OUTPUT_DIRECTORY MODE");
        const std::string mode = argv[2];
        require(mode == "hrtli" || mode == "art" || mode == "hot" || mode == "lits", "unknown adapter mode");
        ...
        if (mode == "hrtli") check<HrtliAdapter>("HRT-LI", base, path.c_str(), keys);
        if (mode == "art") check<ArtAdapter>("ART", base, path.c_str(), keys);
        if (mode == "hot") check<HotAdapter>("HOT", base, path.c_str(), keys);
        if (mode == "lits") check<LitsAdapter>("LITS", base, path.c_str(), keys);
        return 0;
