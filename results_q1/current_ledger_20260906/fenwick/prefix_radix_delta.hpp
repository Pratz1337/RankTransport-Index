...
private:
    struct Node {
        ...
        std::vector<int> child_mass; // Zero-based Fenwick tree, aligned to children.
    };
    ...
    static void rebuild_child_mass(Node* node) {
        node->child_mass.resize(node->children.size());
        for (size_t i = 0; i < node->children.size(); ++i)
            node->child_mass[i] = node->children[i]->subtree_sum;
        for (size_t i = 0; i < node->children.size(); ++i) {
            const size_t parent = i | (i + 1);
            if (parent < node->child_mass.size())
                node->child_mass[parent] += node->child_mass[i];
        }
    }
    ...
    static int assign(Node* node, const std::string& key, size_t pos, int new_weight) {
        ...
        if (index == node->children.size() || first_byte(node->children[index].get()) != byte) {
            ...
            node->child_mass.reserve(node->children.size() + 1);
    ...
    static size_t memory_bytes_node(const Node* node) {
        ...
        total += node->child_mass.capacity() * sizeof(int);
    ...
    static std::pair<bool, int> validate_node(const Node* node) {
        int sum = node->weight;
        int previous = -1;
        if (node->child_mass.size() != node->children.size()) return {false, 0};
        size_t count = 0;
        for (const auto& child : node->children) {
